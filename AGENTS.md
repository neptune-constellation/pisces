...
4. **`src/tui/app.tsx`** — the root Ink component. Owns all state (`entries`, `query`, `selectedIndex`, `defaultConfig`, plus the recent-opens popup state) and centralizes every keyboard shortcut in one `useInput` callback. `Esc`/`Ctrl+C` call Ink's `useApp().exit()` (never `process.exit` directly — that would bypass the terminal cleanup in the entry point). `Ctrl+D`/`Cmd+D` launches the `default` path/command, or opens a blank terminal window when unconfigured. `Ctrl+R`/`Cmd+R` opens the recent-opens popup (loaded from `history/store.ts`); `recordOpen()` is called from every launch path (`handleSelect`, `handleDefaultLaunch`, and the history re-launch). Note: `Ctrl+H` is intentionally NOT used for history — it is the ASCII backspace control character and Ink collapses it into `key.backspace`, indistinguishable from the Backspace key on Windows.
5. **`src/tui/palette.tsx`** — a pure presentational component: search box, results list, scrollbar, hint bar. Holds no app state except the blinking `Cursor` timer. Display/truncation/icon helpers live in `src/tui/display.ts` and are shared with the history popup.
6. **`src/tui/history.tsx`** — the recent-opens popup: a titled, two-line-per-row list (label + path on the first line, `YYYY-MM-DD HH:mm` timestamp dimmed beneath) showing all retained entries (up to 10), with `↑↓ / Enter / Esc` handling delegated to `App`.
7. **`src/history/store.ts`** — the recently-opened cache. `recordOpen()` prepends a `PaletteEntry` snapshot + ISO timestamp and caps the list at 10, persisting to `~/.pisces/.cache/history.json`; `loadHistory()` reads it back leniently (a corrupt file yields `[]`); `formatTimestamp()` renders the local `YYYY-MM-DD HH:mm` string.
8. **`src/tui/banner.tsx`** — the ASCII logo and `PISCES_VERSION`, read dynamically from package.json at runtime (walks up from the module dir to find it).
9. **`src/tui/onboarding.tsx`** — first-run onboarding: automatically detects installed agents (`known-agents.ts`) and editors (`known-editors.ts` — VS Code, PyCharm, IntelliJ IDEA, Qoder, Cursor, and Trae; each resolved via `where` + path hints to prefer the real launcher over other shims, then falling back to default install paths) and writes everything detected into the initial config, with no user prompt; falls back to an empty config when nothing is detected.
